...
class SarcasmDataset(Dataset):
    def __init__(self, data: List[Dict], tokenizer: BertTokenizer, max_length: int = 64):
        """
        Args:
    ...
    def __getitem__(self, index):
        ...
        if text is None:
            return ValueError(f"Data item at index {index} is missing 'headline' key.")
        ...
        return {
            "input_ids": encoded["input_ids"].flatten().to(torch.int64),          
            "attention_mask": encoded["attention_mask"].flatten().to(torch.int64),
